...
public final class StringUtils {
     ...
     * @return
     */
    public static String partition(String string, int start, int end) {
    ...
    public static String doubleToThreeDecimals(double num) {
        DecimalFormat df = new DecimalFormat("0.000");
        return df.format(num);
    }

    public static String doubleToTwoDecimals(int num) {
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(num);
    }
     ...
     * j截取字符串后四位
     */
    public static String interceptingFourBits(String data) {
        if (data.length() >= 4) {// 判断是否长度大于等于4
            return data.substring(data.length() - 4, data.length());//截取两个数字之间的部分
        } else {
            return "";
        }
    }
